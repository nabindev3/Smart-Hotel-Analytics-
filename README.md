...
> ⚠️ **Synthetic data — demonstration project.** This platform trains and evaluates entirely on **synthetic data generated by [`src/generate_data.py`](src/generate_data.py)** (bookings, KPIs, reviews, and regressors are all fabricated). The reported metrics measure how well the models fit a simulation, not real-world performance, and the guest "recommender" learns from a rule-based synthetic interaction matrix rather than real PMS data. It's a portfolio/reference pipeline, not a production system trained on real hotel operations. To run it for real, swap the generated CSVs for PMS/POS exports of the same schema.
